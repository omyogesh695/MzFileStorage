...
# --- STABLE MOTOR CLIENT SETUP WITH POOLING & SSL ---
client = AsyncIOMotorClient(
    Config.MONGO_URI,
    tls=True,
    tlsCAFile=certifi.where(),
    serverSelectionTimeoutMS=5000,   # 5s failover resolution
    connectTimeoutMS=5000,
    socketTimeoutMS=10000,
    maxPoolSize=50,                  # Free tier connection protection
    minPoolSize=5,
    maxIdleTimeMS=60000,             # Dead connection recycling
    retryWrites=True
)
...
async def add_user(user_id: int) -> bool:
    """Adds a new user to the database if they don't exist. Returns True if a new user was inserted."""
    user_data = {
        'user_id': user_id,
        'post_channels': [],
    ...
    result = await users.update_one({'user_id': user_id}, {"$setOnInsert": user_data}, upsert=True)
    return result.upserted_id is not None


# --- OPTIMIZED ATOMIC DAILY STATS ---

async def record_daily_view(owner_id: int, requester_id: int):
    """Atomically records a unique view for an owner for the current day without flooding the connection pool."""
    today_utc = datetime.datetime.utcnow().date()
    today_start = datetime.datetime(today_utc.year, today_utc.month, today_utc.day)

    try:
        # Atomic single-query: creates document if missing and appends unique viewer
        await daily_stats.update_one(
            {
                'owner_id': owner_id,
                'date': today_start,
                'unique_viewers': {'$ne': requester_id}
            },
            {
                '$inc': {'view_count': 1},
        ...
        logger.debug(f"Non-critical stats update pass: {e}")
...
# --- USER VERIFICATION (SHORTENER SYSTEM) ---
...
async def get_post_channel(user_id: int):
    """Retrieves the FIRST post channel ID for backward compatibility."""
    user = await users.find_one({'user_id': user_id})
...
async def save_file_data(owner_id, original_message, copied_message, stream_message):
    """Saves file metadata, including the stream_id."""
...
# --- SMART BACKUP KEYBOARD SERIALIZATION ---
