...
async def handle_public_file_request(client, message, requester_id, payload):
    ...
    if not verified:
        # Check if shortener is configured / enabled for owner
        has_shortener = bool(owner_settings and owner_settings.get("shortener_api") and owner_settings.get("shortener_url"))

        if not has_shortener:
            # ✅ Shortener OFF / Not configured -> Direct Unlock / Send File
            await record_daily_view(owner_id, requester_id)
            return await send_file(client, requester_id, owner_id, file_unique_id)

        # ✅ Shortener ON -> HMAC-Signed Koyeb Destination Link
        ts = int(time.time())
        ...
        shortlink = await get_shortlink(koyeb_destination, owner_id)
