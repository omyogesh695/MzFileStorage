...
async def start_command(client, message):
    ...
    # Check if user is newly inserted in the database
    is_new = await add_user(requester_id)
    log_channel = getattr(Config, "LOG_CHANNEL", None)

    # Trigger alert directly to log channel on new user
    if is_new and log_channel:
        try:
            first_name = message.from_user.first_name or "User"
            user_mention = f"[{first_name}](tg://user?id={requester_id})"
            username_str = f"@{message.from_user.username}" if message.from_user.username else "No Username"

            log_text = (
                f"👤 **#NewUser Started Bot**\n\n"
                f"• **User:** {user_mention}\n"
                f"• **ID:** `{requester_id}`\n"
            ...
            logger.error(f"Failed to send alert to LOG_CHANNEL: {e}")
