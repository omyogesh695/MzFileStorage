...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # 1. Strip Extension & Replace Delimiters
    ...
    # 3. Strip @Channels, Web Links, and known watermarks
    ...
    # 4. Detect Part Number (e.g. part001, part02, pt1, cd1)
    part_info_str = ""
    ...
    # 6. Detect Languages (Prevents "Shang Chi" false trigger for Chinese)
    found_languages = set()
    ...
    # Isolated Check for Chinese/Mandarin
    if re.search(r'\b(chinese|mandarin|cantonese)\b', original_name, re.IGNORECASE):
        found_languages.add("Chinese")

    # Priority audio overrides
    if re.search(r'\bhindi\b', original_name, re.IGNORECASE):
        found_languages.add("Hindi")
    if re.search(r'\benglish\b', original_name, re.IGNORECASE):
        found_languages.add("English")
    ...
    # Safety clean for any residual nested parenthesis
...
async def create_post(client, user_id, messages, cache: dict):
    ...
    # Clean Box Formatting (Prevents Broken Footer Line)
    ...
    footer_length = min(max(len(clean_box_title) + 6, 24), 42)
    footer_line = f"╰{'─' * footer_length}╯"
