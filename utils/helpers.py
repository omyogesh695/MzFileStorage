...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # 2. Extract Source / Rip Type (WEB-DL, WEBRip, HDRip, BluRay, DVD, etc.)
    found_source = ""
    source_patterns = [
    ...
    # 11. Construct Final Display Title
...
async def create_post(client, user_id, messages, cache: dict):
    ...
    # Sort naturally by Quality, Part, and Episode
    media_info_list.sort(key=lambda x: (
        natural_sort_key(x.get('quality_tags', '')),
    ...
    for info in media_info_list:
        ...
        # 5. Audio Languages (Hindi, English, etc.)
