...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # 3. Clean Special Emojis, Symbols, and Delimiters
    clean_base = re.sub(r'[^\w\s\(\)\[\]\.\-_]', ' ', clean_base)
    clean_base = clean_base.replace('_', ' ').replace('.', ' ')

    # 4. Extract Resolution & Languages
    ...
    # 5. Detect TV Season / Episode Info
    season_info_str = ""
    episode_info_str = ""
    ...
    # 6. Extract Year for Movies (cuts off everything after year)
    final_year = None
    ...
    # 7. Clean Junk and Technical Words
    junk_words = [
        ...
        'PROPER', 'iNTERNAL', 'Sample', 'Video', 'Dual', 'Audio', 'Multi', 'WebRip', 'WebDL', 'Web-DL',
    ...
    # 9. Construct Final Display Title
    series_tags = []
    if season_info_str:
...
async def create_post(client, user_id, messages, cache: dict):
    ...
    poster_search_query = first_info['display_title'].split('(')[0].replace(first_info.get('season_info', ''), '').replace(first_info.get('episode_info', ''), '').strip()
