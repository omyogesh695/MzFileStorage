...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # Step A: Series Season & Episode Detection (Matches Season 20, Season_20, S04E07, S020E4, 1x02, etc.)
    se_full_match = re.search(
        r'[\. _\-\[\(](?:season|s)[\s._\-]*0*([1-9]\d*)[\s._\-]*(?:episode|ep|e)[\s._\-]*0*([1-9]\d*)', 
        clean_base, 
        re.IGNORECASE
    )
    season_word_match = re.search(
        r'[\. _\-\[\(](?:season|s)[\s._\-]*0*([1-9]\d*)(?=[^a-z0-9]|$)', 
        clean_base, 
        re.IGNORECASE
    )
    x_match = re.search(r'[\. _\-\[\(]0*([1-9]\d*)x0*([1-9]\d*)', clean_base, re.IGNORECASE)
    range_match = re.search(
        r'[\. _\-\[\(](?:e|ep|episode)[\s._\-]*0*([1-9]\d*)\s*(?:-|to|_)\s*(?:e|ep|episode)?0*([1-9]\d*)', 
        clean_base, 
        re.IGNORECASE
    )
    ...
    if se_full_match:
        is_series = True
        season_info_str = f"S{int(se_full_match.group(1)):02d}"
        episode_info_str = f"E{int(se_full_match.group(2)):02d}"
        cutoff_index = min(cutoff_index, se_full_match.start())
    elif season_word_match:
        is_series = True
        season_info_str = f"S{int(season_word_match.group(1)):02d}"
        cutoff_index = min(cutoff_index, season_word_match.start())
        if ep_only_match and ep_only_match.start() > season_word_match.start():
    ...
    # Step C: Universal Release Tag Cutoff
    meta_barrier = re.search(
        r'[\. _\-\[\(](2160p|1080p|720p|480p|576p|360p|4k|web[\-_]?dl|web[\-_]?rip|bluray|hdrip|dvdrip|hdtc|camrip|ddp\d?|aac\d?|x264|x265|hevc|remux|amzn|nf|hotstar|zee5|bbhin|hs)[\. _\-\]\)]',
        clean_base,
        re.IGNORECASE
    )
    if meta_barrier:
    ...
    title = clean_title_raw.replace('_', ' ').replace('.', ' ')
