...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # 3. Extract Resolution (All standard resolutions including 540p)
    ...
    # 4. Detect Languages
    found_languages = set()
    search_string_lower = clean_base.lower()
    for key, value in LANGUAGE_MAP.items():
        if re.search(r'\b' + key + r'\b', search_string_lower):
            found_languages.add(value)

    # 5. Extract TV Season and Episode Numbers
    season_info_str = ""
    episode_info_str = ""
    show_name_candidate = clean_base

    # Comprehensive pattern checks for all Indian & International formats
    patterns = [
        # Bracket ranges: [E01-E06], [E07-E12], [EP 01 to 08], EP(01-08)
        ...
        # Season X Episode Y (e.g. Season 2 Episode 902, Season 68 Episode 4675)
        (r'\b(?:Season|S)\s*(\d{1,2})\s+(?:Episode|Ep|Epi|E)\s*(\d{1,4})\b', 'se_both'),
        # S01E2128 or S09E847 or S01 EP92
        (r'\bS(\d{1,2})\s*(?:E|EP|Episode)\s*(\d{1,4})\b', 'se_both'),
        # S01 or Season 02 standalone with Episode Y later
    ...
    # Secondary check for standalone season (e.g. "Filing for Love S01 [E01-E06]")
    ...
    junk_words = [
        ...
        'PROPER', 'iNTERNAL', 'Sample', 'Video', 'WebRip', 'WebDL', 'Web-DL', 'AMZN', 'JH', 'HS',
        'Hindi', 'English', 'Tamil', 'Telugu', 'Kannada', 'Malayalam', 'Punjabi', 'Marathi',
        'NF', 'MAX', 'DSNP', 'ZEE5', 'HDRip', 'WEBRip', 'HEVC', 'x265', 'x264', 'AAC',
    ...
    if matched_show:
        ...
        final_year = None
        if default_lang and not found_languages:
            found_languages.add(default_lang)
    else:
    ...
    # 9. Construct Final Display Title (Show Name + Season ONLY)
...
async def create_post(client, user_id, messages, cache: dict):
    ...
    # Poster Search (Base Show/Movie Title)
