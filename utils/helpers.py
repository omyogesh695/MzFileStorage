...
async def clean_and_parse_filename(name: str, cache: dict = None):
    ...
    # 2. Strip @Channel tags, links, and known channel watermarks FIRST
    ...
    # 3. Extract Resolution & Languages
    ...
    # 4. Detect TV Season / Episode Info (including batch brackets like [E07 E12], EP(01-08))
    season_info_str = ""
    episode_info_str = ""
    show_name_candidate = clean_base

    range_patterns = [
        (r'\[\s*E?(\d{1,4})\s*[-_\sTo~–—]+\s*E?(\d{1,4})\s*\]', 'ep_range'),
        (r'EP\((\d{1,4})-(\d{1,4})\)', 'ep_range'),
        (r'\[EP\s*(\d{1,4})\s*to\s*(\d{1,4})\]', 'ep_range'),
        (r'\[Epi\s*(\d{1,4})\s*-\s*(\d{1,4})\]', 'ep_range'),
        ...
        (r'\b(?:Ep|Episode)\s*(\d{1,4})\s*-\s*(\d{1,4})\b', 'ep_range_plain'),
    ]

    for pat, p_type in range_patterns:
    ...
    # Also detect standalone season if extracted separately
    if not season_info_str:
        s_match = re.search(r'\bS(\d{1,2})\b', clean_base, re.IGNORECASE)
        if s_match:
            season_info_str = f"S{int(s_match.group(1)):02d}"

    is_series = bool(season_info_str or episode_info_str)

    # 5. Extract Year for Movies
    final_year = None
    ...
    # 6. Clean Special Emojis and Delimiters
    clean_candidate = re.sub(r'[^\w\s\(\)\[\]\.\-_]', ' ', show_name_candidate)
    clean_candidate = clean_candidate.replace('_', ' ').replace('.', ' ')

    # 7. Clean Junk and Residual Words
    junk_words = [
        'Combined', 'Complete', 'Pack', 'Batch', 'Dl', 'Dual', 'Audio', 'Multi',
        ...
        'PROPER', 'iNTERNAL', 'Sample', 'Video', 'WebRip', 'WebDL', 'Web-DL',
        'Hindi', 'English', 'Tamil', 'Telugu', 'Kannada', 'Malayalam', 'Punjabi', 'Marathi',
        'NF', 'AMZN', 'MAX', 'DSNP', 'ZEE5', 'HDRip', 'WEBRip', 'HEVC', 'x265', 'x264', 'AAC',
        '1tamilmv', 'www', 'mp4', 'mkv', 'avi', '2160p', '1080p', '720p', '480p', '360p',
        r'S\d{1,2}', r'E\d{1,4}'
    ]
    junk_pattern_re = r'\b(' + r'|'.join(junk_words) + r')\b'
    cleaned_candidate = re.sub(junk_pattern_re, '', clean_candidate, flags=re.IGNORECASE)
    ...
    # 8. Check Shortcuts for Indian Serials
    matched_show = None
    ...
    # 9. Construct Final Display Title (Show Name + Season ONLY, NO Episode in Header)
...
async def create_post(client, user_id, messages, cache: dict):
    ...
    media_info_list.sort(key=lambda x: natural_sort_key(x.get('quality_tags', '') or x.get('episode_info', '')), reverse=True)
    first_info = media_info_list[0]
    primary_display_title = first_info['display_title']
    
    # Clean query for Poster Search (Movie / Show name only)
    ...
    for info in media_info_list:
        ...
        # 2. Episode Info (Placed in line details)
